...
    // 1. Strict Bounding Box Check (Fail-Safe mechanism)
    double minX1=1e10, maxX1=-1e10, minY1=1e10, maxY1=-1e10;
    for(const auto& p : t1.polygon) { 
        minX1 = std::min(minX1, p.x); maxX1 = std::max(maxX1, p.x); 
        minY1 = std::min(minY1, p.y); maxY1 = std::max(maxY1, p.y); 
    }
	
    double minX2=1e10, maxX2=-1e10, minY2=1e10, maxY2=-1e10;
    for(const auto& p : t2.polygon) { 
        minX2 = std::min(minX2, p.x); maxX2 = std::max(maxX2, p.x); 
        minY2 = std::min(minY2, p.y); maxY2 = std::max(maxY2, p.y); 
    }

    
    if (maxX1 < minX2 - COLLISION_TOLERANCE || maxX2 < minX1 - COLLISION_TOLERANCE ||
        maxY1 < minY2 - COLLISION_TOLERANCE || maxY2 < minY1 - COLLISION_TOLERANCE) {
        return false;
    }
...
    // CLI Argument parsing designed for Python Orchestrator communication
    for(int i=1; i<argc; i++) {
	...
	// I migrated to Mersenne Twister (mt19937) for a statistically robust uniform distribution compared to legacy rand() 
...
    std::ifstream file("submission.csv");
    if (!file.is_open()) {
        std::cerr << "Error: Cannot open submission.csv\n";
        return 1;
    }
...
    for(auto& t : all_trees) {
        if (t.group_id == target_n) {
            active_group.push_back(&t);
        }
    }
...
        // STRATEGY SETTINGS (Vibration/Shake) 
...
            double current_gravity_str = gravity_strength;
			...
			// I designed a phased approach: aggressively shake out of local minima first, then compact.
            if (k < switch_iteration) { // PHASE 1: SHAKE to South-West
                gravity_mode = 3;
                target_x = -10000.0;
                target_y = -10000.0;
                current_gravity_str = 0.0005;
            } else { // PHASE 2: Compress to Center
                gravity_mode = 0;
...
            if (move_type == 0) {
                t->x += (dist_sym(rng) * step) + pull_x;
            } else if (move_type == 1) {
                t->y += (dist_sym(rng) * step) + pull_y;
            } else {
                t->deg += dist_sym(rng) * 5.0 * step;
            }
